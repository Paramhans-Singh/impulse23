...
window.addEventListener('scroll', function() {
	...
	sections.forEach((current) => {
		...
		if (scrollY > sectionTop && scrollY <= sectionTop + sectionHeight)
			document.querySelector('.menu a[href*=' + sectionId + '] .dot').classList.add('selected');
		else document.querySelector('.menu a[href*=' + sectionId + '] .dot').classList.remove('selected');
	});
});
